...
F(字符串,偏序,DP,倒着DP,局部性)G(题意转化,最小割,最大流)H()
...
然后可以转个45度, 补成个长方形, 原来空白的地方放0,问题变成选一些数的 和 - C * (纵向连续块数+横向连续块数), 这样最大答案不会变
...
如何在100 * 100量级中 选多个长方形, 每个贡献 = Sum - C(长 * 宽)
