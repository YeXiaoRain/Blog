...
https://atcoder.jp/contests/abc248/submissions/35152539
...
struct node{ // 维护 max[l..r] - min[l..r] + l - r
  ...
  array<int,K> cnt; // 最小值+0,+1,+2,+3出现的次数
}t[N*4+10];
...
pair<int,array<int,K>> merge(const pair<int,array<int,K> >&mc0,const pair<int,array<int,K>>&mc1){
  ...
  array<int,K> c ;
  map<int,int> v2c;
  rep(i,0,K) v2c[m0+i] += c0[i];
  rep(i,0,K) v2c[m1+i] += c1[i];
  rep(i,0,K) c[i] = v2c[m + i];
  return {m,c};
}
...
pair<int,array<int,K>> query(int o,int l,int r,int ql,int qr){ // {min, count array}
  ...
  down(o);
  auto ret = pair(0,array<int,K>()); // {min, count array}
