...
pair<LD,int> f(LD v){ // f(斜率) => {最大截距, 横坐标(选的个数)}
  auto dp = vector(n,vector(2,pair<LD,int>{0,0})); // dp[i][选/不选] = {最大截距,个数}
  rep(i,1,n){
    auto [y0, c0] = dp[i-1][0];
...
int main(){
  ...
  LD L = 0, R = 1e15; // 斜率
  rep(t,0,100) { // 100 次二分, 这里可能 有多个点 让同一个斜率最大, 保证 f(R).pos < x <= f(L).pos 即可
    LD mid = (L + R) / 2;
    auto [_, pos] = f(mid);
    if(pos < x) R = mid;
    else L = mid;
  }
  auto [y,_] = f(L); // 注意 可能x 在斜率一致的一段[x0...x1] 的中间,而L得到x0,R的到x1, 但是L,R在斜率上很接近
  ll ans = (ll)(y + x * L);
  printf("%lld\n", ans);
}
```
