...
解释一下,就是n个中选$i = [k\cdots n]$个, 大于x, 而剩余的小于x

根据上面的期望表达式有(这里用了beta function

$\begin{aligned}
E(x) &= \int_{0}^{1} \left\lbrace \sum_{i=m}^n \binom{n}{i} (1-x)^i x^{n-i} \right\rbrace dx \\\\
&= \sum_{i=m}^n \binom{n}{i}  \int_{0}^{1} (1-x)^i x^{n-i} dx \\\\
&= \sum_{i=m}^n \binom{n}{i} \frac{i!(n-i)!}{(n+1)!} \\\\
&= \sum_{i=m}^n \frac{n!}{i!(n-i)!} \frac{i!(n-i)!}{(n+1)!} \\\\
&= \sum_{i=m}^n \frac{1}{n+1} \\\\
&= \frac{n-m+1}{n+1}.
&= 1 - \frac{m}{n+1}.
\end{aligned}$
...
那么`[A,A+1]` 对答案的贡献, $\cdot \sum dp_{n,j = 0 \cdots n, t = 0 \cdots n} \cdot (A + 1 - \frac{k-j}{t+1})$ 

看起来$O(n^4)$ 但是有些状态达不到, 常数一般 

---

看起来还有$O(N^3)$ 甚至 taylor shift 分治可以$N^2 log^2 N$
