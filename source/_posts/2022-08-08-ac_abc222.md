...
这里也说 分治类fft 可以做到 $N log^2 N$, 虽然没试过两个怎么做分治
...
用 Newton’s algorithm 据说可以 $O(N log N)$
...
**这里有一点要用到,但是没有证明的是 $G(F(x)) = x$ 则 $F(G(x)) = x$**, 只能说在$F(x)$的值域内$F(G(F(x)) = F(x)$即$F(G(x)) = x$, 但在值域外不知道..

如果 F,G满足, 且它们0次项系数均为0, 1次项系数均非0

那么有G的n次项 = $(\frac{x}{F(x)})^n$ 的$n-1$次项的$\frac{1}{n}$

记作$\lbrack x^n \rbrack G(x) = \frac{1}{n} \lbrack x^{n-1} \rbrack \left(\frac{x}{F(x)}\right)^n.$
...
$\sum_{i} i (\lbrack x^i \rbrack G (x)) F^{i-1-n} F' = F^{-n}.$

提取$-1$次项目的系数

$\lbrack x^{-1}\rbrack \left(\sum_{i} i (\lbrack x^i \rbrack G (x)) F^{i-1-n} F'\right) = \lbrack x^{-1}\rbrack \left(F^{-n}\right).$
